...
export function OrderDetailPage() {
  ...
  if (order && order.sourceApp !== "Tableside") {
    return (
      <Stack spacing={3}>
        <PageHeader
          eyebrow="Dettaglio"
          title="Ordine non disponibile"
          actions={(
            <Button component={RouterLink} to="/ordini" startIcon={<ArrowBackIcon />}>
              Torna agli ordini
            </Button>
          )}
        />
        <EmptyState
          title="Ordine fuori ambito"
          description="Nel Tableside sono visibili solo gli ordini creati dalla stessa applicazione."
        />
      </Stack>
    );
  }

  const failedAttempts = order?.tickets.flatMap((ticket) => ticket.deliveryAttempts.filter((attempt) => !attempt.success)) ?? [];

  return (
    <Stack spacing={3}>
      <PageHeader
        eyebrow="Dettaglio"
        title={order ? `Ordine #${order.reference}` : "Dettaglio ordine"}
        description="Riepilogo dell'ordine inviato, con voci, selezioni e stato dei ticket cucina generati dal backend."
        actions={(
        ...
        <>
          {failedAttempts.length > 0 ? (
            <Alert severity="warning">
              Sono presenti {failedAttempts.length} tentativi di consegna ticket non riusciti. L'ordine resta comunque registrato.
            </Alert>
          ) : null}
            ...
            <CardContent>
              <Stack spacing={2}>
                <Stack direction={{ xs: "column", sm: "row" }} spacing={2} justifyContent="space-between">
                  <Box>
                    <Typography variant="body2" color="text.secondary">Riferimento</Typography>
                      ...
                      <Card key={ticket.id} elevation={0} sx={{ border: "1px solid rgba(18, 102, 79, 0.1)" }}>
                        <CardContent>
                          <Stack spacing={1.5}>
                            ...
                            <Typography variant="body2" color="text.secondary">
                              Creato {formatDateTime(ticket.createdAt)} · stampato {formatDateTime(ticket.printedAt)} · tentativi {ticket.deliveryAttempts.length}
                            </Typography>
                          </Stack>
                        </CardContent>
                      </Card>
