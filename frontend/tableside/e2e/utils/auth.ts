...
export async function expectLoginPage(page: Page) {
  await expect(page.getByRole("heading", { name: "Accesso Tableside" })).toBeVisible();
}
...
export async function submitLogin(page: Page, credentials: Credentials) {
  ...
  await page.getByRole("button", { name: "Entra nel Tableside" }).click();
}

export async function waitForAuthenticatedShell(page: Page) {
  await expect(page.getByText("Tableside operativo")).toBeVisible();
